...
class GameViewModel(private val repository: GameRepository) : ViewModel() {
     ...
     * unexpected bill, lucky bonus, etc -- and applies its money effect
     * immediately; the UI shows a dismissible card describing what happened. */
    private fun checkDailyLifeEvent() {
        ...
        wallet = when (event.kind) {
            LifeEventKind.BONUS -> wallet.addToman(event.amountToman)
            LifeEventKind.EXPENSE -> wallet.spendToman(event.amountToman) ?: wallet.copy(tomanBalance = 0.0)
        }
        repository.saveWallet(wallet)
